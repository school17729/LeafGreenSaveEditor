...
class Uint8 {
    ...
    public static fromDecimal(value: number): Uint8 {
        ...
        return new Uint8(memory, UintEndianness.BIG);
    }
    ...
    public flipEndian(): number[] {
        const mem: number[] = [];
        for (let i: number = 0; i < Uint8.SIZE; i++) {
            for (let j: number = 0; j < 8; j++) {
                ...
                mem.push(this.memory[(Uint8.SIZE * 8) - (8 * (i + 1)) + j]);
            }
        }
        return mem;
    }
    ...
    public add(other: Uint8): Uint8 {
        const sum: number[] = new Array<number>(Uint8.SIZE * 8);
        ...
        for (let i: number = (Uint8.SIZE * 8) - 1; i >= 0; i--) {
        ...
        return new Uint8(sum, UintEndianness.BIG);
    }
}
